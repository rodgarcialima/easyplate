...
uses
  ...
  {$ENDIF} ecSpell, AppEvnts, ecSingleInst, ecMacroRec, ActiveX,  untEditFrm,
  ecMultRepl;
...
type
  ...
  private
    ...
    procedure SaveFrame(Frame: TfrmEditorFrame; PromtDialog: Boolean = False);
    procedure ApplySharedStyles;
    procedure SetCurrentEditor(const Value: TSyntaxMemo);
    procedure SetCurrentFrame(const Value: TfrmEditorFrame);
    function GetCurrentFrame: TfrmEditorFrame;
    function GetFrameCount: integer;
    function GetFrames(Index: integer): TfrmEditorFrame;
    function GetModifiedCount: integer;
    function PromtToSave(Frame: TfrmEditorFrame; AllowAll: Boolean): TModalResult;
...
{$ENDIF}
  private
    ...
    procedure FrameClosed(Frame: TfrmEditorFrame);
    procedure RegisterNewEditorFrame(Frame: TfrmEditorFrame);
    procedure CreateBind(Frame: TfrmEditorFrame);
  ...
  public
    ...
    function OpenFile(const AFileName: string): TfrmEditorFrame;
    function CreateFrame(const FileName: string): TfrmEditorFrame;
    procedure CloseFrame(Frame: TfrmEditorFrame);

    property CurrentEditor: TSyntaxMemo read FCurrentEditor write SetCurrentEditor;
    property CurrentFrame: TfrmEditorFrame read GetCurrentFrame write SetCurrentFrame;
    property FrameCount: integer read GetFrameCount;
    property Frames[Index: integer]: TfrmEditorFrame read GetFrames;
  published
...
uses ecStrUtils, inifiles, ecExtHighlight, ecEmbObj, untEasySyncEditMainChildFrm;
...
function TfrmEasySyncEditMain.OpenFile(const AFileName: string): TfrmEditorFrame;
var i: integer;
begin
...
procedure TfrmEasySyncEditMain.SaveFrame(Frame: TfrmEditorFrame; PromtDialog: Boolean);
begin
...
function TfrmEasySyncEditMain.PromtToSave(Frame: TfrmEditorFrame; AllowAll: Boolean): TModalResult;
...
var Frm: TfrmEditorFrame;
begin
  with Sender as TfrmEasySyncEditMainMDIChild do
  if ControlCount > 0 then
    begin
      Frm := TfrmEditorFrame(Controls[0]);
      if Frm.Modified then
...
begin
  Action := caFree;
  FrameClosed(TfrmEditorFrame(TForm(Sender).Controls[0]));
end;
...
function TfrmEasySyncEditMain.CreateFrame(const FileName: string): TfrmEditorFrame;
begin
  Result := TfrmEditorFrame.Create(Self);
  Result.Name := '';
...
//生成BPL插件
//取消MDI功能
procedure TfrmEasySyncEditMain.CreateBind(Frame: TfrmEditorFrame);
var tab: TTabSheet;
    frm: TForm;
begin
 { if FMDIView then
    begin
      frm := TfrmEasySyncEditMainMDIChild.Create(Self, Frame);
...
procedure TfrmEasySyncEditMain.RegisterNewEditorFrame(Frame: TfrmEditorFrame);
begin
  with Frame do
    begin
...
procedure TfrmEasySyncEditMain.SetCurrentFrame(const Value: TfrmEditorFrame);
begin
  if FMDIView then
...
function TfrmEasySyncEditMain.GetCurrentFrame: TfrmEditorFrame;
begin
  Result := nil;
  if FMDIView then
    begin
      if ActiveMDIChild <> nil then
        Result := TfrmEditorFrame(ActiveMDIChild.Controls[0])
    end
  else
    begin
      if (PageControl1.ActivePage <> nil) then
        Result := TfrmEditorFrame(PageControl1.ActivePage.Controls[0])
    end;
end;
...
function TfrmEasySyncEditMain.GetFrames(Index: integer): TfrmEditorFrame;
begin
  if FMDIView then
    Result := TfrmEditorFrame(MDIChildren[Index].Controls[0])
  else
    Result := TfrmEditorFrame(PageControl1.Pages[Index].Controls[0])
end;
...
procedure TfrmEasySyncEditMain.FrameClosed(Frame: TfrmEditorFrame);
begin
...
procedure TfrmEasySyncEditMain.CloseFrame(Frame: TfrmEditorFrame);
var ts: TTabSheet;
begin
  CurrentEditor := nil;
      ...
      if FMDIView then
        begin
          ...
          for i := PageControl1.PageCount - 1 downto 0 do
           begin
            CreateBind(TfrmEditorFrame(PageControl1.Pages[i].Controls[0]));
        ...
        begin
          ...
          for i := MDIChildCount - 1 downto 0 do
            begin
              CreateBind(TfrmEditorFrame(MDIChildren[i].Controls[0]));
              MDIChildren[i].Free;
            end;
