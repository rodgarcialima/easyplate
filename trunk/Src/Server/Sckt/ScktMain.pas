...
procedure TSocketForm.N5Click(Sender: TObject);
begin
  Application.MessageBox(PChar('EasyPlate工作平台服务端应用程序' + #13 +
               '' + #13 +
               '★在客户端使用三层结构时,请保持此程序的正常运行★' + #13 +
               '' + #13 +
               '注意事项:'  + #13 +
               '          1、【端口号】是否设置正确' + #13 +
               '          2、【拦截GUID】是否设置,如设置请检查【intrcpt.dll】版本和注册版本是否匹配' + #13 +
               '          3、【线程缓存】大小默认为:10' + #13 +
               '          4、【超时时长】默认为:0表示无限制' + #13 +
               '         ' + #13 +
               '技术支持:一轩软研'), '关于我们', MB_OK +
    MB_ICONINFORMATION);
end;
