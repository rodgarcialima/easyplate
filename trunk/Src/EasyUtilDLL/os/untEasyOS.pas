...
uses
  Windows, Classes, SysUtils, ShellAPI, WinInet, WinSock, Nb30, StdCtrls,
  Controls, Forms;
...
const  
  TWindowsVersion : array [0..8] of String = ('Windows 3.x',   
          'Windows 95',   
          'Windows 98/Me',   
          'Windows Nt 4.0',   
          'Windows 2000',   
  ...
  //获取系统版本
  function GetWindowsVersion: string; stdcall;
  function GetWindowsVersion_dw: DWORD; stdcall;

  //获取计算机用户名
  function GetComputerUserName: PChar; stdcall;
  
  //获取系统用户名
  function GetSystemUserName: PChar; stdcall;
  ...
  function GetPrinterStatus : byte; stdcall;//从并行端口读取打印机状态
  function CheckPrinter : boolean; stdcall;//获取打印机是否出错
    ...
    function GetSystemInfoPtr( const Index : Integer )                            : Pointer;  stdcall;
    //获取指定文件的信息
    ...
    //判断两个文件的创建时间的早晚
    function CompareFileTime(const FileNameOne, FileNameTwo : String; ComparisonType : TTimeOfWhat ): TFileTimeComparision; stdcall;
    //获取文件的创、修改、最后操作时间
    ...
    procedure ShellAbout( const TitleBar, OtherText : String ); stdcall;
    procedure ShutDown; stdcall;

    function FileInfo(const FileName : String ) : TFixedFileInfo; stdcall;

    //EMAIL有效性判断
    procedure SendEMail_Hide; stdcall;
    function CheckMailAddress(Value : string): boolean; stdcall;//判断EMAIL地址是否合法
...
//获得WIN 系统版本号   
function GetWindowsVersion_A: TWinVer;
begin  
  case  Win32Platform of      // SysUtils   
    VER_PLATFORM_WIN32s:      // Windows 3.X平台   
      begin  
        Result.dwVersion := Win3x;   
        Result.sVerSion  := TWindowsVersion[0];   
      end;   
    ...
    VER_PLATFORM_WIN32_WINDOWS:      // Windows 9x/ME 平台   
      ...
      begin  
        ...
        Result.sVerSion  := TWindowsVersion[2];   
      end  
      else  
      begin  
        Result.dwVersion := Win95;   
        Result.sVerSion  := TWindowsVersion[1];   
      end;   
       ...
       case Win32MajorVersion of  
         4:   
          begin  
            ...
            Result.sVerSion  := TWindowsVersion[3];   
          end;   
         5:   
          begin  
            case Win32MinorVersion of    //  2K   
            0:   
              begin  
                Result.dwVersion := Win2000;   
                Result.sVerSion  := TWindowsVersion[4];   
              end;   
            1:   
              begin                       //  XP   
                Result.dwVersion := WinXP;   
                Result.sVerSion  := TWindowsVersion[5];   
              end;   
            2:                           //  2003   
              begin  
                Result.dwVersion := Win2003;   
                Result.sVerSion  := TWindowsVersion[6];   
              end;   
            end;   
          end;   
         6:   
          begin  
            ...
            0:   
              begin  
                ...
                Result.sVerSion  := TWindowsVersion[7];   
              end;   
            1:   
              begin  
                Result.dwVersion := Win2008;   
                Result.sVerSion  := TWindowsVersion[8];   
              end;   
            end;   
          end;   
       end;   
...
var
  wIND: TWinVer;
begin
  wIND := GetWindowsVersion_A;
  Result := wIND.sVerSion;
end;

//返回操作系统版本标识
function GetWindowsVersion_dw: DWORD;
var
  wIND: TWinVer;
begin
  wIND := GetWindowsVersion_A;
  Result := wIND.dwVersion;
end;

  //获取计算机用户名
function GetComputerUserName: PChar;
var
  pcComputer : PChar;
  dwCSize    : DWORD;
begin
  ...
  //获取系统用户名
function GetSystemUserName: PChar;
var
  pcUser   : PChar;
  dwUSize : DWORD;
begin
...
{*获取打印机是否出错,至于收银机的检测，最好问厂家要接口函数或Demo来实现*}
...
function GetSystemInfoPtr( const Index : Integer ): Pointer;
var
  SysInfo : TSystemInfo;
begin
...
{*普通状态发送，引用ShellAPI单元*}
procedure SendEMail_Hide;
var
  Handle : THandle;
begin
  ShellExecute(Handle,'open',PChar('mailto:收件人?subject=主题&cc=抄送人'),nil, nil, SW_SHOWNORMAL);
end;

{*判断EMAIL地址是否合法*}
function CheckMailAddress(Value : string): boolean;
var
  Index: integer;
  lp   : integer;
begin
  Result := false;
  if ((length(trim(Value)) > 20) or (Pos('.', Value) < 4))  or (Pos('.HTM', UpperCase(Value)) > 0)
    or (Pos('.HTML', UpperCase(Value)) > 0)  or (Pos('.ASP', UpperCase(Value)) > 0)
    or (Pos('.JSP', UpperCase(Value)) > 0) then
    exit;
  for lp := 1 to length(Value) do
  if (Ord(Value[lp]) > $80) and (Value[lp] <> '@') then
    exit;
  if (Pos('.', Value) < Pos('@', Value) + 1) then
    exit;
  Index := Pos('@', Value);
  if (Index < 2) or (Index >= Length(Value)) then
    exit;
  Result := true;
end;
